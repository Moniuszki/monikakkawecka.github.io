...
# Let's explore - change data to ctrout?!
...
GGally::ggpairs(
  correlations,
  upper = list(
    continuous = GGally::wrap("cor", size = 7)
  )
) +
  theme(
...
- **Length vs Weight r = 0.774**
- **pH vs Conductivity r = 0.537**
- **Flow vs Elevation r = -0.797**
- **Temperature vs Elevation r = -0.728**
...
```{r}
#| include: false
brown_trout_no_outlier <- brown_trout |>
  filter(Length_mm != 1666)

p1_no_outlier <- ggplot(
  brown_trout_no_outlier,
  aes(
    x = Length_mm,
    y = Weight_g,
    color = County
...
```{r}
#| include: false
#Let's compare plots with and without the longest fish.
...
pH_conductivity_plot <- brown_trout |>
  ggplot(
    aes(
      x = pH,
      y = Conductivity,
  ...
  theme_cowplot()
pH_conductivity_plot
```

```{r}
#| include: false
# Why I see only a few points?
brown_trout |>
...
```{r}
#| include: false
#I have much more fish than points! Do I have some missing data?
brown_trout |>
  summarize(
    pH_NA = sum(is.na(pH)),
    Conductivity_NA = sum(is.na(Conductivity)),
    both_NA = sum(is.na(pH) & is.na(Conductivity)),
    pH_present_Cond_NA = sum(!is.na(pH) & is.na(Conductivity)),
    Cond_present_pH_NA = sum(is.na(pH) & !is.na(Conductivity)),
    pH_Conductivity_present = sum(!is.na(pH)) & sum(!is.na(Conductivity) )
  )
```
...
# Indeed, the higher elevation, the smaller the water flow.Tuolumne has a big range of height. Alpine seems to be a spot that is higher place in total while the rest is spread along diffrenet heights.
```
...
```{r}
#| include: false
ggplot(brown_trout, aes(x = Weight_g)) +
  geom_histogram(
    binwidth = 100,
    fill = "steelblue",
    color = "white"
...
ggplot(brown_trout, aes(x = Length_mm)) +
  geom_histogram(
    binwidth = 25,
    fill = "steelblue",
    color = "white"
  ...
  labs(
    title = "Brown Trout Length Distribution",
    x = "Length (mm)",
    y = "Number of fish"
  ) +
  theme_cowplot()
...
![The fish caught during the trip](images/04774560-ADA9-426B-8B21-D5CA52395994.jpeg)
...
Besides our obvious winner, August and September it the best time to catch a heavy fish!
...
brown_trout |>
  ...
  labs(
    title = "Brown Trout Lenggth by Month",
...
```{r}
#| include: false
#| ## Is Tristan's fish much bigger than what is caught in May?
brown_trout |>
  mutate(
    date = Date,
    month = month(date)
  ...
  labs(
    title = "Weight and Length of Brown Trout Caught in May",
    x = "Length (mm)",
    y = "Weight (g)"
  ) +
  theme_cowplot()
...
Highest Flow (brightest in the Flow plot) Highest Temperature Highest Length Highest Weight Lowest Elevation Lowest pH Lowest Hardness Lowest Conductivity
...
cluster1_vs_tristan <- tibble(
  ...
  `Cluster 1 Mean` = c(
    cluster_summary$Mean_Temperature[cluster_summary$Cluster == 1],
    cluster_summary$Mean_Length[cluster_summary$Cluster == 1],
    cluster_summary$Mean_Weight[cluster_summary$Cluster == 1]
  ),
...
cluster1_vs_tristan |>
  gt() |>
  fmt_number(
    columns = c(`Cluster 1 Mean`, Tristan),
    decimals = 1
  ) |>
  tab_header(
    title = "Comparison of Tristan's Fish with UMAP Cluster 1",
...
ctrout_clustered |>
  ...
  gt() |>
  tab_header(
    title = "Sites and Counties Represented in UMAP Cluster 1",
    subtitle = "Number of fish sampled at each site"
  )
```

Based on my limited data, Tristan's fishing site is the most comparable to Cluster 1.
